...
| **2** | 인트로 | 인트로 | ▪︎ 최초 앱 실행 시, 인트로 화면을 띄우 <br> → 간단한 앱 소개 출력 / 닉네임, 프로필 이미지를 입력 | `3` | |
| **3** | | 로그인<br>(SNS 회원) | ▪︎ SNS 회원의 경우 [애플] 등 소셜 수단으로 로그인할 수 있도록 함 | `2` | - iCloud 동기화 서비스로 제공할 수 있음(미정) |
| **4** | 홈 화면 | 프로필 보기・앱설정 | ▪︎ 소셜 개인 이미지를 볼 수 있도록 함 <br> ▪︎ In-App Purchase 및 구독 관리 기능을 제공하도록 함 | `3` | |
| **5** | | 도서 진척도 보기 | ▪︎ 목표 도서의 진척도 정보를 리스트로 한 눈에 볼 수 있도록 함 <br> → 셀(Cell)에는 도서 표지(Cover), 제목, 부제, 저자, 출판일, 진척도 정보를 간략하게 표시하도록 함 <br> → 목표 도서를 도서 분류 별로 볼 수 있도록 함 <br> ▪︎ 목표 도서의 진척도 정보를 리스트의 순서를 임의로 수정 가능하도록 함 | `1` | |
| **6** |  | 도서 진척도 상세 | ▪︎ 선택 도서의 상세 기능을 제공하고, 목표 달성 현황을 텍스트와 그래프로 한 눈에 볼 수 있도록 함 <br> → ⌜독서 완료⌟, ⌜목표 수정⌟, ⌜목표 삭제⌟ 기능을 제공하고, 완독 목표 달성일까지 D-Day, 일일 독서 현황 막대 그래프 정보를 표시하도록 함 <br> ▪︎ 한줄 글귀 수집 기능으로 인상 깊은 문장을 수집할 수 있도록 함 <br> ▪︎ 페이지 히스토리로 그래프뿐만 아니라 리스트로도 언제 얼마나 읽었는지 볼 수 있도록 함 | `1` | |
| **7** | | 목표 도서 수정(홈 화면) | ▪︎ 목표 도서 정보를 수정할 수 있도록 함 <br> → 완독 목표 날짜 등 | `1` | |
| **8** | | 목표 도서 삭제(홈 화면) | ▪︎ 목표 도서 정보를 삭제할 수 있도록 함 | `1` | |
| **9** | 검색 화면 | 도서 검색 | ▪︎ 국내 도서를 검색하는 기능을 제공하도록 함 <br> → 도서 표지(Cover), 제목, 부제, 분류, 저자, 출판일 정보를 간략하게 표시하도록 함 | `1` | |
| **10** | | 검색 상세 | ▪︎ 검색한 도서 정보를 상세하게 보여주도록 함 <br> → 도서 표지(Cover), 제목, 부제, 분류, 저자, 출판일, 도서 설명 정보를 표시하도록 함 | `1` | |
| **11** | | 목표 도서 추가 | ▪︎ 완독하고자 하는 도서 정보를 추가할 수 있도록 함 <br> → [수동 추가]: 직접 도서 제목 등 정보를 입력해 추가 <br> → [검색 추가]: 알라딘에서 검색한 도서를 추가 <br> → 완독 목표 날짜 등 | `1` | |
| **12** | 책장 화면 | 찜한 도서 보기 | ▪︎ 읽었거나 찜한 도서의 리스트를 볼 수 있도록 함 <br> → 도서 표지(Cover), 제목, 저자를 간략하게 표시하도록 함 | `2` | |
| **13** | | 읽은 도서 보기 | ▪︎ 읽었거나 찜한 도서의 리스트를 볼 수 있도록 함 <br> → 도서 표지(Cover), 제목, 저자를 간략하게 표시하도록 함 | `2` | |
| **14** | | 수집 문장 보기 | ▪︎ 수집한 문장의 리스트를 볼 수 있도록 함 <br> → 도서 표지(Cover), 제목, 저자를 간략하게 표시하도록 함 | `2` | |
| **15** | | 찜한 도서 상세 | ▪︎ 완독 도서의 세부 정보를 볼 수 있도록 함 <br> → 도서 표지(Cover), 제목, 부제 정보를 간략하게 표시하도록 함 | `2` | |
| **16** | | 읽은 도서 상세 | ▪︎ 완독 도서의 세부 정보를 볼 수 있도록 함 <br> → 도서 표지(Cover), 제목, 부제 정보를 간략하게 표시하도록 함 <br> → 목표 달성 여부, 일일 독서 현황 막대 그리프 정보를 표시하도록 함 | `2` | |
| **13** | | 책장 전체 요약 | ▪︎ 읽은 도서의 수, 찜한 도서의 수와 수집 문장의 수를 볼 수 있도록 함 | `2` | |
| **14** | 분석 화면 | 분석 결과 보기 | ▪︎ 지금까지 완독한 모든 도서의 분석 결과를 제공하도록 함 <br> 일일 (전체) 독서 현황 막대 그래프와 연속 독서 스트릭 정보를 표시하도록 함 <br> → 페이지 캘린더로 어느 일자나 얼마나(페이지) 읽었는지 한 눈에 볼 수 있도로 함 | `3` | |
| **15** | 디자인 | 커스텀탭뷰 | ▪︎ 다채로운 애니메이션이 적용된 탭뷰를 적용하도록 함 | `1` | |
...
<img width="1798" alt="스크린샷 2023-03-18 오후 4 28 18" src="https://user-images.githubusercontent.com/21079970/226091665-dba22860-6f98-48bb-993b-080308e6a547.png">
...
| **13** | startDate | Date | 생성 날짜 | |
| **14** | targetDate | Date | 목표 날짜 | | 
| **15** | userRating | Int | 사용자 평점 | |
| **16** | isCompleted | Bool | 완독 여부 | |
...
| **2** | title | String | 도서 제목 | |
| **3** | author | String | 저자/아티스트 | |
| **4** | cover | String | 커버(표지) URL | |
| **5** | isbn13 | String | ISBN-13 | |
...
| 번호 | 구분 | 주 기능 | 상세 기능 | 진척도 | 비고 |
| :--: | :--: | :-- | :-- | :--: | :-- |
| **1** | 인트로 | 스플래시 | ▪︎ 스플래시 로딩 이미지 출력하기 | | | 
|   | | SNS 로그인 | ▪︎ 애플로 로그인하기 | | - iCloud 동기화 서비스를 지원할 수도 있음(미정) | 
| **2** | 프로필 | 프로필 관리 | ▪︎ 프로필 이미지 보기 | | |
|   | | | ▪︎ In-App-Purchase 및 구독 관리하기 | | |
| **3** | 홈 화면 | 활동 내역 보기 | ▪︎ 활동 내역 보기 | `0%` | |
|   | | 목표 도서 보기 | ▪︎ 카테고리 별 보기 <br> ▪︎ 정렬 및 필터링하기 | `100%` | |
|   | | 목표 도서 상세 | ▪︎ 목표 달성 현황 보기 | `90%` | |
|   | | | ▪︎ 독서 데이터 보기(그래프) | `90%` | |
|   | | | ▪︎ 독서 데이터 보기(리스트) | `90%` | |
|   | | 목표 도서 수정 | ▪︎ 목표 도서 정보 수정하기 (목표 완독일 등) | `0%` | |
|   | | 목표 도서 삭제 | ▪︎ 목표 도서 정보 삭제하기 | `90%` | |
|   | | 문장 수집 기능 | ▪︎ 수집 문장 추가하기 | | |
|   | | | ▪︎ 수집 문장 수정하기 | | |
|   | | | ▪︎ 수집 문장 삭제하기 | | |
| **6** | 검색 | 도서 검색 | ▪︎ 도서 검색하기 (제목/저자) | `100%`|  |
|   | | 검색 상세 | ▪︎ 검색 도서 상세 보기 | `100%` | |
|   | | 목표 도서 추가 | ▪︎ 목표 도서 추가하기 <br> → [수동 추가]: 직접 도서 제목 등 정보를 입력해 추가 <br> → [검색 추가]: 알라딘에서 검색한 도서를 추가 | `60%` | |
| **7** | 책장 | 찜한 도서 보기 | ▪︎ 찜한 도서 보기  <br> → 검색 및 필터링 기능 등 | `100%` | |
|   | | 읽은 도서 보기 | ▪︎ 읽은 도서 보기  <br> → 검색 및 필터링 기능 등 | `0%` | |
|   | | 수집 문장 보기 | ▪︎ 수집 문장 보기  <br> → 검색 및 필터링 기능 등 | | |
|   | | 찜한 도서 상세 | ▪︎ 찜한 도서 상세 보기 | `100%` | |
|   | | 읽은 도서 상세 | ▪︎ 읽은 도서 상세 보기 | `0%` | |
|   | | 수집 문장 상세 | ▪︎ 수집 문장 상세 보기 |  | |
| **8** | 분석 | 분석 결과 | ▪︎ 모든 읽은 도서 분서 결과 보기 <br> → 일일 독서 현황 막대 그래프 및 연속 독서 스트릭 정보 보기 등 | | |
|   | | | ▪︎ 페이지 캘린더 보기 | | |
| **9** | 디자인 | 커스텀 탭뷰 | ▪︎ 커스텀 탭뷰 적용 | `100%` | |
