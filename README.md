...
| 10 | 기능<br>(책장) | 완독・좋아요 도서 보기 | ▪︎ 완독하거나 좋아요를 체크(Check)한 도서의 리스트를 볼 수 있도록 함(LazyVGrid) <br> → 도서 표지(Cover), 제목, 부제 정보를 간략하게 표시하도록 함 <br> → 도서 표지(Cover), 제목, 부제 정보를 간략하게 표시하도록 함 | `1` | |
| 11 | 기능<br>(분석) | 분석 결과 보기 | ▪︎ 지금까지 완독한 모든 도서의 분석 결과를 제공하도록 함 <br> → 읽고 있는 도서와 완독한 도서의 개수, 일일 (전체) 독서 현황 막대 그래프와 연속 독서 스트릭 정보를 표시하도록 함 | `2` | |
