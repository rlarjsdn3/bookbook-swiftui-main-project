...
|   | | SNS 로그인 | ▪︎ 애플 등 | | | 
| **2** | 프로필 | 프로필 관리 | ▪︎ SNS의 프로필 이미지 보기 | | |
|   | | | ▪︎ In-App-Purchase 및 구독 관리 | | |
| **3** | 홈 | 목표 도서 보기 | ▪︎ 분류 별 보기 | | |
|   | | 목표 도서 정렬 | ▪︎ 리스트 정렬 및 순서 바꾸기 | | |
|   | | 목표 도서 상세 | ▪︎ 목표 달성 현황을 텍스트와 그래프로 보기 | | |
|   | | | ▪︎ 도서 평점(추가/수정/삭제) | | |
|   | | | ▪︎ 문장 수집(추가/수정/삭제) | | |
| **4** | 도서 | 목표 도서 추가 | ▪︎ 목표 도서 추가(완독 목표일, 사용자 테마) <br> → [수동 추가]: 직접 도서 제목 등 정보를 입력해 추가 <br> → [검색 추가]: 알라딘에서 검색한 도서를 추가(ContextMenu) | | |
|   |  | 목표 도서 수정 | ▪︎ 목표 도서 수정(상동) | | |
|   |  | 목표 도서 삭제 | ▪︎ 목표 도서 정보 삭제(상동) | |  |
| **6** | 검색 | 도서 검색 | ▪︎ 도서 검색(도서 명/저자 명) | `80%`|  |
|   | | 검색 상세 | ▪︎ 검색 도서 상세 보기 | `60%` | |
| **7** | 책장 | 좋아요・완독 도서 보기 | ▪︎ 완독 혹은 좋아요 도서 보기 <br> → 토글 스위치(Toggle)를 통해 완독 혹은 좋아요 도서 목록 중 하나를 표시하도록 함 | | |
|   | | 좋아요・완독 도서 상세 | ▪︎ 완독 혹은 좋아요 도서 상세 정보 보기 | | |
| **8** | 분석 | 분석 결과 보기 | ▪︎ 총 완독 도서 분서 결과 보기 <br> → 읽고 있는 도서와 완독한 도서의 개수, 일일 (전체) 독서 현황 막대 그래프와 연속 독서 스트릭 정보를 표시하도록 함 | | |
|   | | | ▪︎ 페이지 캘린더 보기 | | |
